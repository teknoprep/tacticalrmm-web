...
/* Theme-aware. Quasar puts .body--dark on <body> in dark mode.
   Light mode: the toolbar is brand cyan and the login card is white, so the near-black
   mark is used -- white measures 2.03:1 on cyan and 1.00:1 on a white card, both failing.
   Dark mode: surfaces are near-black, so the white mark is used. */
.q-toolbar__title {
...
.q-toolbar__title::before {
  ...
  margin-right: 10px;
  flex: 0 0 auto;
  background: url("blueuc-logo-dark.png") left center / contain no-repeat;
}
.body--dark .q-toolbar__title::before {
  background-image: url("blueuc-logo-reverse.png");
}
...
.q-card__section .text-h4.ellipsis::before {
  ...
  height: 54px;
  margin: 0 auto 12px;
  background: url("blueuc-logo-dark.png") center center / contain no-repeat;
}
.body--dark .q-card__section .text-h4.ellipsis::before {
  background-image: url("blueuc-logo-reverse.png");
}
"""
...
def install_logo_css(dist):
    ...
    for src_name, out_name in (("logo-reverse.png", "blueuc-logo-reverse.png"),
                               ("logo-dark.png", "blueuc-logo-dark.png"),
